...
Later on we will show examples of how to read data from a source file, eg. csv, parquet, excel, or even a different database, directly into motherduck but for now let's assume you want to upload some data that you already have loaded in your R environment.
...
One limitation of the connecting to motherduck from R is that you first need to create a local motherduck instance and then connect to motherduck which means you have access to both local and cloud based instances. 
...
If we want to we can also navigate to a different database you can use the `cd()`command
...
```{r}
md_con <- connect_to_motherduck()

md_con |> show_current_user()

list_schemas(md_con)

DBI::dbGetQuery(md_con,"select * from duckdb_tables()")
DBI::dbGetQuery(md_con,"SELECT distinct *
FROM information_schema.schemata;")


dbGetQuery(md_con, "PRAGMA database_list;")


dbGetQuery(md_con, "show pg_settings;")
```
-   from parquet
